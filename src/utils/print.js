function openPrintWindow(html, opts = {}) {
  ...
  var css = [
    "@page { margin: 10mm 12mm; size: legal " + (opts.landscape ? "landscape" : "portrait") + "; }",
  ...
  var page = "<!DOCTYPE html><html><head><meta charset='utf-8'/><title>" + (opts.title || "Impression DLC") + "</title><style>" + css + "</style></head><body>" + html + "</body></html>";
  w.document.open();
...
// Imprime du HTML fourni directement (ex: listes/tableaux générés dynamiquement,
// sans passer par un élément #print-zone du DOM).
export function printHTML(html, opts) {
  openPrintWindow(html, opts);
}
